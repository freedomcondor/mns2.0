function create_1drone_4pipuck_children_node(droneDis, pipuckDis, height, position, orientationQ, tilt_nose)
...
function create_left2_right1_line_morphology(scale, droneDis, pipuckDis, height)
	...
	node.children[10].split = true
	return node
end

function create_main_morphology()
	local pipuckDis = 0.7
	local droneDis = 1.5
	local height = 1.8
...
function create_ranger_morphology()
	local pipuckDis = 0.7
	local droneDis = 1.5
	local height = 1.8
	local node = create_3drone_12pipuck_children_node(droneDis, pipuckDis, height, vector3(), quaternion())
