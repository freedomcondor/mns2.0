...
function api.droneAdjustHeight(z)
	...
	if true then
		...
		logger("checking height, current height = ", currentHeight == nil, currentHeight, "test")
		logger(currentHeight)
...
function api.droneEstimateHeight()
	...
	if average_count ~= 0 then
		...
		logger("EstimateHeight returns value, average_count = ", average_count, "average_height = ", average_height)
		return average_height
	else
		logger("EstimateHeight returns nil")
		return nil
	end
end
