...
local state = 1
local left_obstacle_type = 32
local right_obstacle_type = 34
local target_type = 28
...
return function()
	...
	if vns.parentR == nil then
		...
		for i, ob in ipairs(vns.avoider.obstacles) do
			if ob.positionV3.y > middle and ob.type == left_obstacle_type and
			   ob.positionV3.y < left then
				left = ob.positionV3.y
			end
			if ob.positionV3.y < middle and ob.type == right_obstacle_type and
		...
		elseif left > 9 and right > -9 then
			SpeedY = 1
			logger("left_bias")
		elseif left < 9 and right < -9 then
			SpeedY = -1
...
return function()
	if vns.api.stepCount == 500 then 
		if (robot.id == "drone2") or 
		   (robot.id ~= "drone3" and robot.random.uniform() < 0.3) then
			fail_return = false
			if vns.robotTypeS == "drone" then
