...
package.path = package.path .. ";C:/Users/rosha/OneDrive/Desktop/Weixu/mns2.0/build/testing/exp_0_hw_05_gate_switch/?.lua"
...
function step()
	...
	logger(api.droneTags)
	logger("obstacles")
	logger(vns.avoider.obstacles)
	--[[
	logger("goal = ", vns.goal.positionV3, vns.goal.orientationQ)
	logger("seen tags = ")
	logger(api.droneTags)
	logger("obstacles")
	logger(vns.avoider.obstacles)
	logger("virtualFrame")
	logger(api.virtualFrame.orientationQ)
	logger("X = ", vector3(1,0,0):rotate(api.virtualFrame.orientationQ))
	logger("Y = ", vector3(0,1,0):rotate(api.virtualFrame.orientationQ))
	logger("Z = ", vector3(0,0,1):rotate(api.virtualFrame.orientationQ))
	--]]
	...
	return function()
		...
		elseif state == "move_forward" then
			...
			if vns.parentR == nil then
				...
				if receiveWall ~= nil then
					...
					if gateNumber == totalGateNumber and disToTheWall < 0.5 then
					...
					if #vns.avoider.obstacles ~= 0 then
						...
						for id, ob in ipairs(vns.avoider.obstacles) do
							if ob.type ~= wall_brick_type and ob.type ~= gate_brick_type then
								number = number + 1
		...
		elseif state == "check_gate" then
			...
			if gate ~= nil and vns.robotTypeS == "drone" then
				...
				logger(robot.id, "I have a gate, breaking and recruiting, gate length", gate.length)
			else
				if vns.robotTypeS == "drone" then
					logger(robot.id, "I'm a drone but I see no gate")
				end
			end
