...
def generate_drone_vertebrate_locations(scale, origin_x, origin_y) :
	a = []
	a.append([origin_x, origin_y])
	a.append([origin_x + 0.3, origin_y + 1])
	a.append([origin_x + 0.3, origin_y - 1])
	for i in range(1, scale) :
		a.append([origin_x + i + 0.3, origin_y])
		a.append([origin_x + i + 0.3, origin_y + 1])
		a.append([origin_x + i + 0.3, origin_y - 1])
	return a
...
def generate_drone_square_locations(n_side, origin_x, origin_y) :
	a = [[origin_x, origin_y]]
	for i in range(0, n_side) :
		for j in range(0, n_side) :
			if i != 0 or j != 0:
				a.append([origin_x + 0.4 + i, origin_y + 0.4 + j])
	return a
...
def generate_drone_pipuck_xml(drone_n, scale,
                              origin_x,    origin_y, 
                              x_min_limit, x_max_limit,
                              y_min_limit, y_max_limit, 
	...
	drone_locations = generate_drone_locations(drone_n, 
	                                           ...
	                                           drone_near_limit, drone_far_limit) 
	'''
	drone_locations = generate_drone_vertebrate_locations(scale, origin_x, origin_y)
	i = 0
...
drone_pipuck_xml = generate_drone_pipuck_xml(N, Scale,             # number of drone
...
generate_structure("morphology_chain", Scale-1)
