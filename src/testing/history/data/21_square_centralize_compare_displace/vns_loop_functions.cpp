...
if (DISPLACE == 1) {
      UInt32 unRemoveStep = 400;
      UInt32 unAddinStep = 420;
      CRange<Real> rangeX = CRange<Real>(-2.2, 2.2);
      CRange<Real> rangeY = CRange<Real>(-2.2, 2.2);
      Real removePositionX = -8;
      Real removePositionY = -8;
      UInt32 unDroneNumber = 25;

      if (m_unStepCount == unRemoveStep) {
         int robot_index = 6;
         bool flag = false;
         while (flag == false) {
            robot_index = m_pcRNG->Uniform(CRange<UInt32>(1, unDroneNumber+1)); // 1 to 15
            robot_name.str(""); 
   ...
   REGISTER_LOOP_FUNCTIONS(CVNSLoopFunctions, "vns_loop_functions_data_21_displace");
