...
function api.droneAdjustHeight(z)
	...
	if true then
			...
			if z == 0 then
				...
				if api.actuator.newPosition.z > threshold then
					api.actuator.newPosition.z =
						...
						api.actuator.newPosition.z - 0.02 * 0.2
				end
		...
		api.actuator.newPosition.z = robot.flight_system.position.z + heightError * 0.2 * ZScalar
...
function api.droneSetSpeed(x, y, z, th)
	...
	x = x * transScalar * 0.2
	y = y * transScalar * 0.2
	z = z * transScalarZ * 0.2
	th = th * rotateScalar * 0.2
...
function api.droneDetectTags()
	...
	for _, camera in pairs(robot.cameras_system) do
		for _, newTag in ipairs(camera.tags) do
			...
			if (vector3(0,0,1):rotate(orientationQ) - vector3(0,0,1)):length() > 0.5 then
