...
function Stabilizer.step(vns)
	if vns.parentR ~= nil then
		...
		local step_offset_in_ref = {
			positionV3 = vector3(-vns.stabilizer.step_offset.positionV3):rotate(vns.stabilizer.step_offset.orientationQ:inverse()),
			orientationQ = vns.stabilizer.step_offset.orientationQ:inverse()
		}
		vns.stabilizer.reference_offset.positionV3 = 
			vns.stabilizer.reference_offset.positionV3 + vector3(step_offset_in_ref.positionV3):rotate(
		...
		vns.stabilizer.reference_offset.orientationQ = vns.stabilizer.reference_offset.orientationQ * 
		                                               step_offset_in_ref.orientationQ

		-- accumulate reference_offset to goal
		vns.goal.positionV3 = vns.stabilizer.reference.positionV3 + 
		                      vector3(vns.stabilizer.reference_offset.positionV3):rotate(vns.stabilizer.reference_offset.orientationQ)
...
function Stabilizer.getNearestReference(vns)
	...
	if nearest == nil then
		...
		for idS, robotR in pairs(vns.connector.seenRobots) do
			if robotR.positionV3:length() < distance then
