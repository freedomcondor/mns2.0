...
function Allocator.step(vns)
		...
		for _, branchT in ipairs(vns.allocator.target.children) do
			...
			if branchT.actorS == nil then
				...
				if nearestId ~= nil then
					...
					vns.childrenRT[nearestId].goal.positionV3 = branchT.positionV3
					vns.childrenRT[nearestId].goal.orientationQ = branchT.orientationQ
					vns.Msg.send(nearestId, "branch", {target = branchT})
				end
			else
				vns.childrenRT[branchT.actorS].goal.positionV3 = branchT.positionV3
				vns.childrenRT[branchT.actorS].goal.orientationQ = branchT.orientationQ
			end
		end
	end
	...
	for idS, childVns in pairs(vns.childrenRT) do
		if vns.allocator.allocated_children[idS] == nil and
		   childVns.assignTargetS == nil then
			childVns.goal.positionV3 = vector3()
			childVns.goal.orientationQ = quaternion()
		end
	end
	...
	for idS, childVns in pairs(vns.childrenRT) do
		if childVns.assignTargetS ~= nil and vns.childrenRT[childVns.assignTargetS] ~= nil then
			childVns.goal.positionV3 = vns.childrenRT[childVns.assignTargetS].positionV3 
		elseif childVns.assignTargetS ~= nil and vns.parentR ~= nil and childVns.assignTargetS == vns.parentR.idS then
			childVns.goal.positionV3 = vns.parentR.positionV3 
		end
	end
end
