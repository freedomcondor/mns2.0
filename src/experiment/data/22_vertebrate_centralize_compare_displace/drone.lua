...
function step()
	...
	if centralize_flag ~= true then
		...
		if api.stepCount < 20 then
			vns.allocator.mode_switch = "stationary"
		elseif api.stepCount < 21 or api.stepCount == 260 then
			vns.allocator.mode_switch = "allocate"
		elseif robot.id == "drone1" then
			vns.allocator.mode_switch = vns.centralize_mode
		end
		if api.stepCount == 220 then
			local children_count = 0
			for idS, robotR in pairs(vns.childrenRT) do
			...
			if children_count == 0 then
				vns.centralize_mode = "stationary"
			end
		end
	end
