...
casefolder = 'experiment/data/08_full_scenario3'
...
for i in range(test_start, test_end + 1):
	...
	jumpfile = open("jumpStep.csv","r")
	jumps = []
	for line in jumpfile:
		jumps.append(float(line))

	os.system(testfolder_build + "/evaluator/main " + str(jumps[0]) + " ./")
	os.system("mv result.txt " + testfolder_src + "/random/run" + str(i) + "/result1.txt")
	os.system(testfolder_build + "/evaluator/main " + str(jumps[1]) + " ./")
	...
	os.system("mv result.txt " + testfolder_src + "/random/run" + str(i) + "/result3.txt")
