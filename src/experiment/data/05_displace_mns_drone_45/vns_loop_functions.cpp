...
if (DISPLACE == 1) {
      ...
      if (m_unStepCount == unRemoveStep) {
         ...
         bool flag = false;
         while (flag == false) {
            robot_index = m_pcRNG->Uniform(CRange<UInt32>(1, 46)); // 1 to 15
            robot_name.str(""); 
            ...
            for(SDrone& s_drone: m_vecDrones) {
               if (s_drone.Entity->GetId() == robot_name.str()) {
                  ...
                  if (robot_index != 16) { // not root
                  //if (robot_index == 32) { // second hand missing
      ...
      if (m_unStepCount == unAddinStep) {
         ...
         for(SDrone& s_drone: m_vecDrones) {
            if (s_drone.Entity->GetId() == robot_name.str()) {
               bool flag = false;
               while (flag == false) {
                  Real x_number = m_pcRNG->Uniform(CRange<Real>(-8.0, 8.0));
                  Real y_number = m_pcRNG->Uniform(CRange<Real>(-2.0, 2.0));
   ...
   REGISTER_LOOP_FUNCTIONS(CVNSLoopFunctions, "vns_loop_functions_05_displace_drone_45");
